...
def evaluate(Delta, homegas):
    ...
    xy = np.vstack((np.imag(hks), np.real(hks))).transpose()
...
if __name__ == "__main__":
    ...
    plt.plot(homegas, np.real(fv_hks), lw=2, color=cmap[0], label="2nd order FV")

    plt.figure(2)
    plt.plot(homegas, 0 * homegas, lw=2, color=cmap[-1])
    plt.plot(homegas, np.imag(fv_hks), lw=2, color=cmap[0], label="2nd order FV")
    ...
    plt.figure(1)
    ax = plt.gca()
    plt.xlabel(r"$\overline{\omega} h$", fontsize=22, fontweight="bold")
    ...
    plt.savefig("spatial_dispersion.png", format="png", dpi=300)
    ...
    plt.setp(ax.get_xmajorticklabels(), fontsize=18, fontweight="bold")
    plt.setp(ax.get_ymajorticklabels(), fontsize=18, fontweight="bold")
    plt.savefig("spatial_diffusion.png", format="png", dpi=300)

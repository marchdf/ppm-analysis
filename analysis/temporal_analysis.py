...
def evaluate(homega, hks):
    ...
    xy = np.vstack((np.imag(homegas), np.real(homegas))).transpose()
...
if __name__ == "__main__":
    ...
    p = PatchCollection(polygons, edgecolors=cmap[0], linewidths=2, facecolors="none")
    ax.add_collection(p)

    plt.figure(1)
    plt.plot(hks, hks, lw=2, color=cmap[-1])
    plt.plot(hks, np.real(fv_homegas), lw=2, color=cmap[0], label="2nd order FV")

    plt.figure(2)
    plt.plot(hks, 0 * hks, lw=2, color=cmap[-1])
    plt.plot(hks, np.imag(fv_homegas), lw=2, color=cmap[0], label="2nd order FV")
    ...
    for i, CFL in enumerate(CFLs):
        ...
        p = PatchCollection(
            polygons, edgecolors=cmap[i + 1], linewidths=2, facecolors="none"
        )
        ax.add_collection(p)
    ...
    plt.figure(0)
    ax = plt.gca()
    plt.xlabel(r"$\Re(i\overline{\omega}^*h)$", fontsize=22, fontweight="bold")
    plt.ylabel(r"$\Im(i\overline{\omega}^*h)$", fontsize=22, fontweight="bold")
    plt.axis("equal")
    ...
    plt.ylabel(r"$\Re(\overline{\omega}^*h)$", fontsize=22, fontweight="bold")
    plt.xlim([0, np.pi])
    plt.ylim([0, np.pi])
    ...
    ax = plt.gca()
    plt.xlabel(r"$kh$", fontsize=22, fontweight="bold")
    plt.ylabel(r"$\Im(\overline{\omega}^*h)$", fontsize=22, fontweight="bold")
    plt.xlim([0, np.pi])
    # plt.ylim([0, np.pi])
    legend = ax.legend(loc="best")
    plt.setp(ax.get_xmajorticklabels(), fontsize=18, fontweight="bold")

...
def epsilon(x):
    ...
    CFL = x[0]
    kh = x[1]
    return 1. / np.pi * (kh + (3. * CFL**2 / 4 - 5 * CFL / 12 - 4. / 3) * np.sin(kh)
...
def gamma(x):
    ...
    CFL = x[0]
    kh = x[1]
    return 1. / (-2) * (4. * CFL**2 / 3 - 7. * CFL / 3 + (-23. * CFL**2 / 12 + 35 * CFL / 12) * np.cos(kh)
...
if __name__ == '__main__':
    ...
    # alphas = np.sort(np.concatenate([np.logspace(-6, 0, 50),
    #                                  np.linspace(3e-1, 1, 20)]))
    # df = pd.DataFrame(columns=['alpha', 'CFL', 'kh', 'objective'])
    # df.alpha = alphas
    # for k, alpha in enumerate(df.alpha):
    #     constraints = ({'type': 'eq',
    #                     'fun': lambda x: np.array([epsilon(x) - alpha])},
    #                    {'type': 'ineq',
    ...
    alphas = np.sort(np.concatenate([np.logspace(-3, 0, 50),
                                     np.linspace(3e-1, np.pi + 2, 20)]))[::-1]
    df = pd.DataFrame(columns=['alpha', 'CFL', 'kh', 'objective'])
    df.alpha = alphas
    x0 = [1.0, np.pi]
    for k, alpha in enumerate(df.alpha):
    ...
    extent = [0, np.pi, 0, 1]
    levels = np.logspace(-6, 0, 13)
    im = plt.imshow(gamma_error,
                    aspect='auto',
                    interpolation='bilinear',
                    origin='lower',
                    extent=extent,
                    cmap='Blues_r')
    plt.contour(gamma_error,
                levels,
                colors='gray',
                origin='lower',
                extent=extent,
    ...
    plt.savefig('pareto_dispersion.png', format='png')
    ...
    plt.setp(ax.get_xmajorticklabels(), fontsize=18, fontweight='bold')
    plt.setp(ax.get_ymajorticklabels(), fontsize=18, fontweight='bold')
    plt.savefig('pareto_dispersion2.png', format='png')
    ...
    plt.setp(ax.get_xmajorticklabels(), fontsize=18, fontweight='bold')
    plt.setp(ax.get_ymajorticklabels(), fontsize=18, fontweight='bold')
    plt.savefig('pareto_diffusion.png', format='png')
    ...
    plt.setp(ax.get_xmajorticklabels(), fontsize=18, fontweight='bold')
    plt.setp(ax.get_ymajorticklabels(), fontsize=18, fontweight='bold')
    plt.savefig('total_error.png', format='png')

    plt.figure(5)
    ax = plt.gca()
    plt.xlabel(r"CFL", fontsize=22, fontweight='bold')
    plt.ylabel(r"$\int \epsilon \mathrm{d}(hk)$",
               fontsize=22,
               fontweight='bold')
    plt.xlim([0, 1])
    plt.setp(ax.get_xmajorticklabels(), fontsize=18, fontweight='bold')
    plt.setp(ax.get_ymajorticklabels(), fontsize=18, fontweight='bold')
    plt.savefig('int_dispersion_error.png', format='png')

    plt.figure(6)
    ax = plt.gca()
    plt.xlabel(r"CFL", fontsize=22, fontweight='bold')
    plt.ylabel(r"$\int \gamma \mathrm{d}(hk)$",
               fontsize=22,
               fontweight='bold')
    plt.xlim([0, 1])
    plt.setp(ax.get_xmajorticklabels(), fontsize=18, fontweight='bold')
    plt.setp(ax.get_ymajorticklabels(), fontsize=18, fontweight='bold')
    plt.savefig('int_diffusion_error.png', format='png')
